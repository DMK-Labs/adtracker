...
(defn simple-process [keywords include-related?]
  (let [uc  (map #(.toUpperCase %) keywords)
        kws (if include-related?
              (map :relKeyword (rel-kws {:keywords (remove empty? uc)}))
              uc)]
    (-> (first-place kws)
        (set/join (nth-place 2 kws) {:keyword :keyword :device :device})
        (set/join (nth-place 3 kws) {:keyword :keyword :device :device})
        (set/join (nth-place 4 kws) {:keyword :keyword :device :device})
        (set/join (nth-place 5 kws) {:keyword :keyword :device :device})
        (set/join (median-bid-stats-both-channels kws))
        (set/join (min-bid-stats-both-channels kws)))))
